...
class Plausichecks extends FHCAPI_Controller
{
	...
	public function runChecks()
	{
		...
		foreach ($this->_fehlerLibMappings as $fehler_kurzbz => $libName)
		{
			...
			if (isError($fehlerRes)) $this->terminateWithError(getError($fehlerRes));
			...
			if (isError($plausicheckRes)) $this->terminateWithError(getError($plausicheckRes));
					...
					if (!isEmptyString($fehler->fehlertext))
					{
						...
						if (!isEmptyArray($fehlertext_params))
						{
							...
							if (count($fehlertext_params) != substr_count($fehlerText, '%s'))
								$this->terminateWithError('Wrong number of parameters for Fehlertext, fehler_kurzbz ' . $fehler_kurzbz);
		...
		$this->terminateWithSuccess($allIssues);
	}
}
